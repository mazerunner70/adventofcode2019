...
class IntCodeInterpreterV4(memory: Memory) extends MtProcess {
  ...
  def instructionAdd(parameterModes: List[Char], params: List[Long]): Int = {
    def operation = (x: Long, y: Long) => x + y
  ...
  override def checkCanRun(): Boolean = (!console.inputList.isEmpty || memory.programCounter == 0) && !isEnded
}
