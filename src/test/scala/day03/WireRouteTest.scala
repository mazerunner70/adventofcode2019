...
class WireRouteTest extends FlatSpec {
  ...
  it should "shortestIntersection1" in {
    ...
    val (wireAPath, wireBPath, intersections) = wireRoute.getIntersections(routeStringA, routeStringB)
    assert(wireRoute.shortestIntersection(intersections) == 159)
  }
  it should "shortestIntersection2" in {
    ...
    val wireRoute = new WireRoute()
    val (wireAPath, wireBPath, intersections) = wireRoute.getIntersections(routeStringA, routeStringB)
    assert(wireRoute.shortestIntersection(intersections) == 135)
  }
